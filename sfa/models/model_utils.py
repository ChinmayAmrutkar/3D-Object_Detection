...
<<<<<<< HEAD
=======
from models import cbam_fpn_resnet
...
=======
        raise ValueError("Architecture format must include number of layers, e.g., cbam_fpn_resnet_18")

    if 'cbam_fpn_resnet' in configs.arch:
        print('Using CBAM-Enhanced ResNet with FPN')
        model = cbam_fpn_resnet.get_pose_net(num_layers=num_layers,
                                             heads=configs.heads,
        ...
        print('Using standard ResNet architecture')
        model = resnet.get_pose_net(num_layers=num_layers,
                                    heads=configs.heads,
                                    head_conv=configs.head_conv,
                                    imagenet_pretrained=configs.imagenet_pretrained)
    else:
